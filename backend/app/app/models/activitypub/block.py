...
class Block(Base):
    """
    Blocking of one account by another. This is not a domain-level block.
    """
    ...
    actor: Mapped[Optional["Actor"]] = relationship(back_populates="blocking", foreign_keys=[actor_id])
    target_id: Mapped[str] = mapped_column(ForeignKey("actor.id"))
    target: Mapped["Actor"] = relationship(back_populates="blocked_by", foreign_keys=[target_id])
