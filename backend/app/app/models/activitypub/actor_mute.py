...
class ActorMute(Base):
    """
    Muting of one account by another.
    """
    ...
    actor: Mapped[Optional["Actor"]] = relationship(back_populates="muting", foreign_keys=[actor_id])
    target_id: Mapped[str] = mapped_column(ForeignKey("actor.id"))
    target: Mapped["Actor"] = relationship(back_populates="muted_by", foreign_keys=[target_id])
    notifications: Mapped[bool] = mapped_column(default=False, nullable=True)
