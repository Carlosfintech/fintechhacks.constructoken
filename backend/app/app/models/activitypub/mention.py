...
class Mention(Base):
    """
    'Tagging' or 'mention' of an actor within a post.
    """
    ...
    actor: Mapped[Optional["Actor"]] = relationship(back_populates="mentioned", foreign_keys=[actor_id])
    target_id: Mapped[str] = mapped_column(ForeignKey("actor.id"))
    target: Mapped["Actor"] = relationship(back_populates="mentions", foreign_keys=[target_id])
    status_id: Mapped[str] = mapped_column(ForeignKey("status.id"))
    status: Mapped["Status"] = relationship(back_populates="mentions", foreign_keys=[status_id])
    notify: Mapped[bool] = mapped_column(default=True, nullable=True)  # For automatically notifying creator
