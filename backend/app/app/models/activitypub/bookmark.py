...
class Bookmark(Base):
    """
    'Bookmark' by an actor of a post.
    """
    ...
    actor: Mapped[Optional["Actor"]] = relationship(back_populates="bookmarked", foreign_keys=[actor_id])
    target_id: Mapped[str] = mapped_column(ForeignKey("actor.id"))
    target: Mapped["Actor"] = relationship(back_populates="bookmarks", foreign_keys=[target_id])
    status_id: Mapped[str] = mapped_column(ForeignKey("status.id"))
    status: Mapped["Status"] = relationship(back_populates="bookmarks", foreign_keys=[status_id])
