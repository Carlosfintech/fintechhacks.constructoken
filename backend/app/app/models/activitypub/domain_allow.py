...
class DomainAllow(Base):
    ...
    creator: Mapped[Optional["Creator"]] = relationship(back_populates="domain_allows", foreign_keys=[creator_id])
    private_comment: Mapped[str] = mapped_column(nullable=True)  # visible to mods only
    text: Mapped[dict[str | Locale, "DomainAllowText"]] = relationship(
...
class DomainAllowText(Base):
    ...
    #       domainallow.text[locale]
    # Where `locale` is a Babel type.
    id: Mapped[str] = mapped_column(String(26), primary_key=True, index=True, default=generate_ULID)
    domainallow_id: Mapped[str] = mapped_column(ForeignKey("domainallow.id", onupdate="CASCADE", ondelete="CASCADE"))
    domainallow: Mapped["DomainAllow"] = relationship(back_populates="text", foreign_keys=[domainallow_id])
    text: Mapped[Optional[str]] = mapped_column(nullable=True)
    language: Mapped[Locale] = mapped_column(LocaleType, nullable=True)

    def __init__(self, language: str | Locale, text: str, back_ref: DomainAllow | None = None):
        self.language = language
        self.text = text
        if back_ref:
            self.domainallow = back_ref
