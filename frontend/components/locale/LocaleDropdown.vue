  ...
  <Listbox as="div">
    <div class="relative mt-2 w-36">
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
        ...
        <ListboxOptions
          class="absolute -translate-y-full -bottom-7 z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
...
<script setup lang="ts">
  ...
  function setLocale(term: string) {
    currentLocale.value = supportedLocales.find((l) => l.iso === term || l.code === term || l.language === term) as any
  }
